...
async function seedData(tableName: string, filePath: string) {
  ...
  const formattedTableName = pluralize.singular(
    tableName.charAt(0).toUpperCase() + tableName.slice(1) // Capitalize first letter
  );
  ...
  for (const item of data) {
    try {
      await dynamoose.model(formattedTableName).create(item); // Use the model to create an item
    } catch (err) {
      console.error(
...
// This function will delete all tables and then create new ones
// and seed them with data from JSON files located in the "data" directory
// The JSON files should be named after the tables they correspond to
// For example, if you have a table named "users", the JSON file should be named "users.json"
export default async function seed() {
  await deleteAllTables();
  await new Promise((resolve) => setTimeout(resolve, 1000)); // Wait for tables to be deleted
  await createTables();
