...
export default function DroppableComponent() {
  ...
  const handleSectionDragEnd = (result: any) => {
    ...
    const startIndex = result.source.index; // The index of the section being dragged
    const endIndex = result.destination.index; // The index where the section is dropped

    // Create a copy of the sections array
    const updatedSections = [...sections];

    // Remove the dragged section from its original position
    // and store it in reorderedSection
    const [reorderedSection] = updatedSections.splice(startIndex, 1);

    // Insert the dragged section at its new position
    // Update the sections state with the reordered sections
    // and dispatch the action to update the Redux store
    // This will trigger a re-render of the component with the new order
